...
.card{background:var(--card);border-radius:16px;padding:12px 14px;margin:8px 0;
...
function render(ev){
  switch(ev.type){
    case 'run_start':
      return '<div class="card">'+agentHead(ev,'任务')+'<p class="text">'+esc(ev.text)+'</p></div>';
    case 'step':
    ...
    case 'tool_result':{
      ...
      return '<div class="card"><div class="trow">'+tool+'<b>'+esc(ev.tool_name)+'</b>'+fail+meta(ev)+'</div>'
    ...
    case 'delegate_start':{
      ...
      return '<div class="card"><div class="trow">'+out+'<b>委派 → '+esc(ev.tool_name)+'</b>'+meta(ev)+'</div>'
    ...
    case 'delegate_end':{
      ...
      return '<div class="card"><div class="trow">'+back+'<b>委派返回</b>'
...
function poll(){
  fetch('/api/trace').then(function(r){return r.json();}).then(function(d){
    ...
    if(d.events.length>seen){window.scrollTo(0,document.body.scrollHeight);}
